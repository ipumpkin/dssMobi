Ext.define('Dss.controller.Main', {
	...
	config : {
		...
		control : {
			...
			monthMenu : {
				itemtap : 'onMenuTap',
				back : 'onMonthMenuBack'
			},
			dayMenuDateSelect : {
				tap : 'showDayDatePicker'
			},
			monthMenuDateSelect : {
				tap : 'showMonthDatePicker'
			},
			datePicker : {
				change : 'dateChanged'
			}
	...
	showMonthReportById : function(id) {
		var nav = this.getMonthMenu();
		var view = nav.getStore().getNodeById(id);
		
		this.showMonthView(view);
		this.setCurrentDemo(view);
	},
	showMonthView : function(item) {
		var nav = this.getMonthMenu(), layout = nav.getLayout(), anim = item.get('animation'), initialAnim = layout
		.getAnimation(), newAnim;
		
		this.getMonthMenuDateSelect().setText(Global.curMonthDate);
		this.getMonthMenuDateSelect().show();
		var grid = Global.viewCache[item.get("MENU_ID")];
		if (grid) {
			...
			grid = Ext.create('Ext.ux.touch.grid.List', {
				...
				store : {
					...
					proxy : {
						...
						extraParams : {
							key : item.get("REPORT_KEY"),
							'params["month_id"]' : Global.curMonthDate,
							'params["latn_id"]' : Global.latn_id
						},
						reader : {
							type : 'json',
	...
	showDayDatePicker : function() {
		this.getDatePicker().setValue({
			year : parseInt(Global.curDayDate/100),
			month : Global.curDayDate%100
		});
	...
	showMonthDatePicker : function() {
		this.getDatePicker().setValue({
			year : parseInt(Global.curMonthDate/100),
			month : Global.curMonthDate%100
		});
